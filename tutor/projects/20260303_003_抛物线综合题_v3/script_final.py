"""
抛物线综合题 - 最终版（纯英文MathTex + 中文Text分离）
"""
from manim import *
import json
...
class ParabolaProblem(Scene):
    ...
    def scene_1_title(self):
        ...
        problem = VGroup(
            ...
            MathTex(r"B(3, 0),\; x = 1", font_size=28, color="#f39c12"),
            MathTex(r"(1)\; \text{Find } y = ?", font_size=26, color="#2ecc71"),
            MathTex(r"(2)\; \angle PCB = \angle ABC,\; \text{Find } P", font_size=26, color="#2ecc71")
    ...
    def scene_2_find_a(self):
        ...
        step1 = Text("Step 1: Find point A", font_size=36, color="#3498db")
    ...
    def scene_3_equation(self):
        ...
        step2 = Text("Step 2: Solve coefficients", font_size=36, color="#3498db")
        
        eq1 = MathTex(r"a - b = -4", font_size=28)
        eq2 = MathTex(r"-\frac{b}{2a} = 1", font_size=28)
        ...
        content = VGroup(step2, eq1, eq2, sol, final).arrange(DOWN, buff=0.5).center()
    ...
    def scene_4_answer(self):
        ...
        title = Text("Answer", font_size=44, color="#3498db")
        
        ans1 = MathTex(r"(1)\; y = -\frac{4}{3}x^2 + \frac{8}{3}x + 4", font_size=28)
        ans2 = MathTex(r"(2)\; P_1(2, 4) \text{ or } P_2(\frac{7}{3}, \frac{20}{9})", font_size=26)
