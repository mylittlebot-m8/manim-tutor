...
修复：
1. 第 1 幕：已知和求解左右分开显示
2. 第 2-9 幕：标题在上方，画图在左下，公式在右下
3. 标题框高度从 30% 改为 18%，给画图和公式更多空间，为字幕留出空间
"""
...
class GeometryRotationProblem(Scene):
    ...
    def show_layout(self, title_text=""):
        ...
        if title_text:
            # 标题分成左右两部分，分别对应画图和公式区域
            # 左标题（画图区域上方）
            left_title = Text(title_text, font_size=32, color=WHITE)
            left_title.move_to(self.DIAGRAM_CENTER + UP * 3.5)
            
            # 右标题（公式区域上方，留空）
            right_title = Text("", font_size=32, color=WHITE)
            right_title.move_to(self.FORMULA_CENTER + UP * 3.5)
            
            self.play(FadeIn(left_title))
            self.title_text = left_title
    ...
    def update_title(self, text):
        """更新标题文字"""
        new_title = Text(text, font_size=32, color=WHITE)
    ...
    def scene_1_intro(self):
        ...
        problem.move_to(RIGHT * 5 + UP * 1.5)
