...
1. 背景边框恢复原始大小（不放大）
2. 标题位置恢复原始位置（不上移）
3. 第 1 幕：已知/求解 Y 坐标往下调（使用 DOWN 不是 UP）
4. 第三问换行问题：调整 Tex 内容格式
"""
...
class GeometryRotationProblem(Scene):
    ...
    def construct(self):
        ...
        # 下部左右对半分（恢复原始大小）
        self.DIAGRAM_CENTER = LEFT * 4.75 + DOWN * 1.4  # 画图区域中心（恢复原始）
        self.FORMULA_CENTER = RIGHT * 4.75 + DOWN * 1.4  # 公式区域中心（恢复原始）
        
        # 第 1 幕内容位置（往下调，使用 DOWN）
        self.KNOWN_POS = LEFT * 3.5 + DOWN * 0.5  # 已知条件位置（往下调）
        self.PROBLEM_POS = RIGHT * 3.5 + DOWN * 0.5  # 求解问题位置（往下调）
        
        # 标题位置（恢复原始）
        self.TITLE_POS = self.DIAGRAM_CENTER + UP * 3.5  # 标题在画图区域上方
    ...
    def scene_2_rotation(self):
        ...
        # 右侧：提示（严格在右侧区域，解决换行问题）
        hint = Tex(
            r"\textbf{思路：}\triangle CDE \text{可看作} \triangle CDB",
            r"\text{绕点} D \text{旋转} 90^\circ \text{得到}",
            font_size=26,
            color="#f39c12"
    ...
    def scene_3_proof(self):
        ...
        proof = Tex(
            r"\text{在} \triangle ACD \text{和} \triangle BED \text{中：}",
            r"CD = DE\text{（已知）}",
            r"\angle ADC = \angle BDE\text{（对顶角）}",
            r"\angle ACD = \angle BDE\text{（旋转）}",
            r"\therefore \triangle ACD \cong \triangle BED\text{（SAS）}",
    ...
    def scene_4_q2(self):
        ...
        # 右侧：结论
        result = Tex(
            r"\text{由（1）：} \triangle ACD \cong \triangle BED",
            r"\therefore BE = AC = 4",
            font_size=32,
            color="#2ecc71"
        ).arrange(DOWN, buff=0.5)
    ...
    def scene_6_q3_similar(self):
        ...
        hint = Tex(
            r"\textbf{关键：}",
            r"\text{证明} \triangle ACD \sim \triangle CBD",
            font_size=28,
            color="#f39c12"
        ).arrange(DOWN, buff=0.5)
        hint.move_to(self.FORMULA_CENTER)
    ...
    def scene_7_q3_proof(self):
        ...
        # 右侧：证明（严格右对齐，trim 空白）
        proof = Tex(
            r"\text{在 Rt} \triangle CDE \text{中：}",
            r"CE^2 = CD^2 + DE^2 = 2CD^2",
            r"",
            r"\textbf{证明} CD^2 = BD \cdot AD\textbf{：}",
            r"\triangle ACD \sim \triangle CBD\text{（AA）}",
            r"\therefore \frac{AD}{CD} = \frac{CD}{BD}",
            r"\text{即} CD^2 = BD \cdot AD",
            r"",
    ...
    def scene_8_q3_minimum(self):
        ...
        calc = Tex(
            r"\text{由①：} CE^2 = 2BD \cdot AD",
            r"\text{设} AD = x\text{，则} BD = 5 - x",
            r"CE^2 = 2(5-x)x = -2x^2 + 10x",
            r"",
            r"\text{二次函数，开口向下，有最大值}",
            r"\text{当} x = 2.5 \text{时，} CE^2_{max} = 12.5",
            r"",
    ...
    def scene_9_summary(self):
        ...
        summary = Tex(
            r"\textbf{答案：}",
            r"(1) \triangle ACD \cong \triangle BED\text{（SAS）} \Rightarrow \angle ABE = 90^\circ",
            r"(2) BE = 4",
            r"(3) \text{①} CE^2 = 2BD \cdot AD",
            r"\text{②} CE_{min} = \frac{5\sqrt{2}}{2}",
            r"",
            r"\textbf{解题方法：}",
            r"1. \text{旋转全等} \rightarrow \text{证明垂直}",
            r"2. \text{勾股定理} \rightarrow \text{求边长}",
            r"3. \text{相似三角形} \rightarrow \text{建立关系}",
            r"4. \text{二次函数} \rightarrow \text{求最值}",
            r"",
            r"\textbf{谢谢观看！}",
            font_size=22
