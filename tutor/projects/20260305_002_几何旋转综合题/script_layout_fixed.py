...
class GeometryRotationProblem(Scene):
    ...
    def construct(self):
        ...
        # 定义精确布局（使用坐标而非相对位置）
        # 屏幕：x[-7.11, 7.11], y[-4, 4]
        self.TITLE_Y = 2.8  # 题目区域中心 Y 坐标
        self.DIAGRAM_CENTER = LEFT * 3.5 + DOWN * 1.4  # 画图区域中心
        self.FORMULA_CENTER = RIGHT * 3.5 + DOWN * 1.4  # 公式区域中心
    ...
    def create_layout(self):
        ...
        self.title_box.move_to(UP * self.TITLE_Y)
        ...
        self.title_text.next_to(self.title_box, RIGHT, buff=0.5).align_to(self.title_box, UP).shift(DOWN * 0.5 + LEFT * 6)
    ...
    def show_layout(self, title_text=""):
        ...
        if title_text:
            ...
            new_title.move_to(self.title_box.get_center())
            self.play(FadeIn(new_title))
            self.title_text = new_title
    
    def hide_layout(self):
        """隐藏布局框架"""
        self.play(FadeOut(self.title_box), FadeOut(self.diagram_box), FadeOut(self.formula_box), FadeOut(self.title_text))
    
    def update_title(self, text):
        """更新标题文字"""
        new_title = Text(text, font_size=36, color=WHITE)
        new_title.next_to(self.title_box, RIGHT, buff=0.5).align_to(self.title_box, UP).shift(DOWN * 0.5 + LEFT * 6)
    ...
    def scene_1_intro(self):
        ...
        known.next_to(main_title, DOWN, buff=0.5).align_to(main_title, LEFT).shift(RIGHT * 0.5)
        ...
        problem.next_to(main_title, DOWN, buff=0.5).align_to(main_title, RIGHT).shift(LEFT * 0.5)
