...
几何旋转综合题 - 精确定位布局版 v3
修复：
1. 第 1 幕：已知/求解 Y 坐标往下调，求解区域往左调，避免内容换行奇怪
2. 添加 ASS 字幕烧录支持
"""
...
class GeometryRotationProblem(Scene):
    ...
    def construct(self):
        ...
        # 第 1 幕内容位置（往下调，往中间调）
        self.KNOWN_POS = LEFT * 3.5 + UP * 2.0  # 已知条件位置
        self.PROBLEM_POS = RIGHT * 3.5 + UP * 2.0  # 求解问题位置
    ...
    def show_layout(self, title_text=""):
        ...
        if title_text:
            # 标题在画图区域上方
            new_title = Text(title_text, font_size=32, color=WHITE)
