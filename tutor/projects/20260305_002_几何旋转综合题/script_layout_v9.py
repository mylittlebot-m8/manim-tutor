...
几何旋转综合题 - 精确定位布局版 v9
根据用户反馈修改：
1. 去掉第二幕开始的背景框红蓝线（只保留边框，去掉填充）
2. 公式文字区域：往左 20%，往上 20%，大小加大 30%
3. 检查重复文字问题（减少文字数量）
"""
...
class GeometryRotationProblem(Scene):
    ...
    def construct(self):
        ...
        # 标题位置：往上 72px（1080p 中 72px ≈ 0.067）
        self.TITLE_POS = self.DIAGRAM_CENTER + UP * 3.567
        
        # 公式区域起始位置：往左 20%，往上 20%
        # 往左 20%: LEFT * 0.2 * 9.0 = LEFT * 1.8
        # 往上 20%: UP * 0.2 * 6.5 = UP * 1.3
        # 所以新位置 = RIGHT * 4.75 + LEFT * 1.8 + DOWN * 1.4 + UP * 1.3
        # = RIGHT * 2.95 + DOWN * 0.1
        self.FORMULA_START_POS = RIGHT * 2.95 + DOWN * 0.1
        
        # 字体大小加大 30%：从 24 到 31，从 28 到 36，从 32 到 42
        self.FONT_SMALL = 31
        self.FONT_MEDIUM = 36
        self.FONT_LARGE = 42
    ...
    def create_layout(self):
        # 标题框（只保留边框，去掉填充）
        self.title_box = Rectangle(width=18, height=self.TITLE_HEIGHT, color="#3498db", stroke_width=2, fill_opacity=0)
        self.title_box.to_edge(UP, buff=0.2)
        
        # 画图框（只保留边框，去掉填充）
        self.diagram_box = Rectangle(width=9.0, height=6.5, color="#2ecc71", stroke_width=2, fill_opacity=0)
        self.diagram_box.move_to(self.DIAGRAM_CENTER)
        
        # 公式框（只保留边框，去掉填充）
        self.formula_box = Rectangle(width=9.0, height=6.5, color="#e74c3c", stroke_width=2, fill_opacity=0)
    ...
    def scene_2_rotation(self):
        ...
        ).arrange(DOWN, buff=0.5 * 1.3)  # buff 加大 30%
    ...
    def scene_3_proof(self):
        ...
        ).arrange(DOWN, buff=0.35 * 1.3)  # buff 加大 30%
    ...
    def scene_5_q2_calc(self):
        ...
        calc = VGroup(
            ...
            MathTex(r"AB^2 = AC^2 + BC^2 = 4^2 + 3^2 = 25", font_size=self.FONT_MEDIUM),
            MathTex(r"\therefore AB = 5", font_size=self.FONT_MEDIUM),
            Text("", font_size=self.FONT_MEDIUM),
    ...
    def scene_8_q3_minimum(self):
        ...
        calc = VGroup(
            ...
            Text("", font_size=self.FONT_MEDIUM),
            MathTex(r"\therefore CE_{min} = \sqrt{12.5} = \frac{5\sqrt{2}}{2}", font_size=self.FONT_MEDIUM, color="#2ecc71")
        ).arrange(DOWN, buff=0.35 * 1.3)
        calc.move_to(self.FORMULA_START_POS)
    ...
    def scene_9_summary(self):
        ...
        summary = VGroup(
            ...
            MathTex(r"(1)\ \triangle ACD \cong \triangle BED\ (SAS)\ \Rightarrow\ \angle ABE = 90^\circ", font_size=self.FONT_SMALL),
            MathTex(r"(2)\ BE = 4", font_size=self.FONT_SMALL),
            MathTex(r"(3)\text{①}\ CE^2 = 2BD \cdot AD", font_size=self.FONT_SMALL),
            MathTex(r"\text{②}\ CE_{min} = \frac{5\sqrt{2}}{2}", font_size=self.FONT_SMALL),
            Text("", font_size=self.FONT_SMALL),
