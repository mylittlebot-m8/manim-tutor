...
class TeachingScene(Scene):
    ...
    def _get_problem_text_from_audio(self):
        """从 audio_info.json 读取题目文本"""
        # 检查 audio_info.json
        try:
            audio_info_path = self.audio_dir / "audio_info.json"
            print(f"[DEBUG] 查找 audio_info: {audio_info_path}, 存在：{audio_info_path.exists()}")
            
            if audio_info_path.exists():
                with open(audio_info_path, "r") as f:
                    audio_info = json.load(f)
                    print(f"[DEBUG] audio_info 内容：{audio_info}")
                    
                    # 返回第一个音频的描述作为题目
                    if audio_info.get("files"):
                        text = audio_info["files"][0].get("text", "数学题目")
                        print(f"[DEBUG] 题目文本：{text[:50]}...")
                        return text
