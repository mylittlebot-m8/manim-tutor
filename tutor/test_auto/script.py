...
class TeachingScene(Scene):
    ...
    def construct(self):
        ...
        try:
            ...
            # 默认时长（秒）
            self.audio_timings = {i: 10.0 for i in range(1, 20)}
    ...
    def play_scene_1(self):
        """第1幕：题目朗读"""
        # 添加音频
        audio_file = self.script_dir / "audio" / "audio_001_题目朗读.wav"
    ...
    def play_scene_2(self):
        """第2幕：问题分析"""
        audio_file = self.script_dir / "audio" / "audio_002_问题分析.wav"
        if audio_file.exists():
            self.add_sound(str(audio_file))
    ...
    def play_scene_3(self):
        """第3幕：建立模型"""
        audio_file = self.script_dir / "audio" / "audio_003_建立模型.wav"
        if audio_file.exists():
            self.add_sound(str(audio_file))
    ...
    def play_scene_4(self):
        """第4幕：求解过程"""
        audio_file = self.script_dir / "audio" / "audio_004_求解过程.wav"
        if audio_file.exists():
            self.add_sound(str(audio_file))
    ...
    def play_scene_5(self):
        """第5幕：验证答案"""
        audio_file = self.script_dir / "audio" / "audio_005_验证答案.wav"
        if audio_file.exists():
            self.add_sound(str(audio_file))
    ...
    def play_scene_6(self):
        """第6幕：总结"""
        audio_file = self.script_dir / "audio" / "audio_006_总结.wav"
        if audio_file.exists():
            self.add_sound(str(audio_file))
